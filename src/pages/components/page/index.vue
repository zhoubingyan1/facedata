...
<template>
    <div class="page">
        <slot/>
        <Icon type="md-arrow-dropleft" class="prev" :class="{'disabled':active==1}" @click="_goTo(-1)" />
        <span class="pagespan" title="1" @click="_goTo(1)" :class="{'active':active==1}" v-if="active>=showNum">1</span>
        <span class="pagespan jump-prev" :title="titleTipP" @click="_jumpTo(-1)" v-if="active>=showNum"></span>
        <span class="pagespan" :title="item" v-for="(item,index) in lists" :key="index" v-text="item" :class="{'active':item==active}"
           @click="_goTo(item)"></span>
        <span class="pagespan jump-next" :title="titleTipN"  @click="_jumpTo(1)" v-if="jumpNext"></span>
        <span class="pagespan" :title="pageCount" @click="_goTo(pageCount)" :class="{'active':active==pageCount}"
           v-if="pageCount>=1">{{pageCount}}</span>
        <Icon type="md-arrow-dropright" class="next" :class="{'disabled':pageCount<=active}" @click="_goTo(0)" />
...
.page{
  ...
  .prev,.next{
      height: 20px; 
        ...
        cursor: pointer;
        // padding:4px 6px;
        &:hover{background: rgba(233,240,252,1); color: rgba(36,11,234,1) ;}
  }
  .pagespan{ 
    ...
    &.active{color: #fff; background: #246FEA }
